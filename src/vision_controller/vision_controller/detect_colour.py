...
class DetectColourNode(Node):
	...
	def __init__(self):
		...
		self.declare_parameters(
			...
			parameters=[
				...
				("hue2_lower", [170, 120, 70]), # lower range for the second colour (if needed)
				("hue2_upper", [180, 255, 255]) # upper range for the second colour (if needed)
			]
		)
	...
	def detect_colour(self, cv_image):
		...
		hue1_lower = np.array(self.get_parameter("hue1_lower").get_parameter_value().integer_array_value, dtype=np.uint8)
		hue1_upper = np.array(self.get_parameter("hue1_upper").get_parameter_value().integer_array_value, dtype=np.uint8)
		...
		if hue2_lower_param and hue2_upper_param:
			hue2_lower = np.array(self.get_parameter("hue2_lower").get_parameter_value().integer_array_value, dtype=np.uint8)
			hue2_upper = np.array(self.get_parameter("hue2_upper").get_parameter_value().integer_array_value, dtype=np.uint8)
